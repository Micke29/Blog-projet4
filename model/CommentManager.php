...
class CommentManager extends Manager
{
	...
	{
		$db = $this->dbConnect();
		$comments = $db->prepare('SELECT cmt_id, cmt_author, cmt_content, DATE_FORMAT(cmt_date, \'%d/%m/%Y à %Hh%imin\') AS cmt_date_fr, cmt_report FROM t_comments_cmt WHERE pst_id = ? ORDER BY cmt_date ASC');
	...
	{
		$db = $this->dbConnect();
		$req = $db->prepare('DELETE FROM t_comments_cmt WHERE cmt_id = ?');
		$req->execute(array($id));
	}

	public function approuveReportComment($id)
	{
		$db = $this->dbConnect();
		$req = $db->prepare('UPDATE t_comments_cmt SET cmt_report = 0 WHERE cmt_id = ?');
		$req->execute(array($id));
	}
}